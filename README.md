...
| `common.extraEnv`                  | Array with extra environment variables to add to all pods as YAML                                                                                                                                                                                        | `[]`  |
...
| `application.extraEnv`                                      | Array with extra environment variables to add to all pods as YAML                                                                                                                                     | `[]`            |
...
| `cronjob.extraEnv`                 | Array with extra environment variables to add to all pods as YAML                                                                                                                                                                                                                                                                                         | `[]`        |
...
| `cronjob.volumes`                  | Optionally specify extra list of additional volumes for the cronjob pod(s)                                                                                                                                                                                                                                                                                | `[]`        |
| `cronjob.volumeMounts`             | Optionally specify extra list of additional volumeMounts for the cronjob container(s)                                                                                                                                                                                                                                                                     | `[]`        |
...
| `keda.initialReplicas`  | Explicit initial `.spec.replicas` for the Deployment when KEDA is enabled (use 0 for scale-to-zero-from-cold). When null, replicas is omitted so KEDA owns the replica count as before. Requires the consuming ArgoCD app to set ignoreDifferences on the Deployment's /spec/replicas, otherwise a self-heal reconcile reverts replicas after KEDA scales. | `nil`   |
