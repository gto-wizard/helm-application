...
| `keda.initialReplicas`  | Explicit initial `.spec.replicas` for the Deployment when KEDA is enabled (use 0 for scale-to-zero-from-cold). When null, replicas is omitted so KEDA owns the replica count as before. | `nil`   |
